...
func init() {
	...
	//----------------------------------------------------------------------------
  	// Migrate the schema
  	//----------------------------------------------------------------------------
  	err := utils.GetDB().AutoMigrate(&model.Matchup{}).Error
  	
  	if  (err == nil ) {
  		fmt.Println( strings.ToTitle( "Finished AutoMigrate on Matchup" ) )
  	} else {
  	    fmt.Println( strings.ToTitle( "Failed to AutoMigrate on Matchup" ), err )
  	}
}
