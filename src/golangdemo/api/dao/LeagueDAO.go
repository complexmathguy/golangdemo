...
func init() {
	...
	//----------------------------------------------------------------------------
  	// Migrate the schema
  	//----------------------------------------------------------------------------
  	err := utils.GetDB().AutoMigrate(&model.League{}).Error
  	
  	if  (err == nil ) {
  		fmt.Println( strings.ToTitle( "Finished AutoMigrate on League" ) )
  	} else {
  	    fmt.Println( strings.ToTitle( "Failed to AutoMigrate on League" ), err )
  	}
}
